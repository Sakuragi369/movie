...
def noise_arcs(draw, image):
    size = image.size
    draw.arc([-20, -20, size[0], 20], 0, 295, fill=settings.CAPTCHA_NOISE_COLOR[random.randint(0, 2)])
    draw.line([0, 20, size[0] + 20, size[1] - 50],width=1, fill=settings.CAPTCHA_NOISE_COLOR[random.randint(0, 2)])
    draw.line([-20, 0, size[0] + 20, size[1]],width=1, fill=settings.CAPTCHA_NOISE_COLOR[random.randint(0, 2)])
    return draw
...
def noise_dots(draw, image):
    ...
    for p in range(int(size[0] * size[1] * 0.1)):
        draw.point((random.randint(0, size[0]), random.randint(0, size[1])), fill=settings.CAPTCHA_NOISE_COLOR[random.randint(0, 2)])
    return draw
